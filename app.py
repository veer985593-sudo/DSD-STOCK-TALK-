...
Mobile Optimized & Corrected Breakout Logic
"""
...
st.set_page_config(
    ...
    page_icon="🚩",
    layout="wide",
    initial_sidebar_state="collapsed", # Sidebar default hidden
)

# --- 💎 PREMIUM CSS (Optimized for Mobile) ---
st.markdown("""
<style>
    ...
    div[role="tablist"] button p { color: #94A3B8 !important; font-size: 1rem !important; }
    ...
    /* Hide the default Sidebar completely as we moved everything to main */
    [data-testid="collapsedControl"] { display: none; }
</style>
""", unsafe_allow_html=True)

# --- 🚀 F&O STOCKS MASTER LIST ---
FO_STOCKS_LIST = [
...
def _render_custom_metric(label, value, change, pct):
    ...
    <div style="background: rgba(30, 41, 59, 0.7); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 12px; padding: 15px; box-shadow: 0 4px 6px rgba(0,0,0,0.3); margin-bottom: 10px;">
        ...
        <div style="color: #FFFFFF !important; font-size: 1.5rem; font-weight: 800;">{value:,.2f}</div>
...
def _render_alert(message, type="success"):
    ...
    html = f'<div style="background: {bg}; border: 1px solid {border}; border-radius: 8px; padding: 12px 16px; margin-bottom: 15px; color: {text_color} !important; font-weight: 500; font-size: 0.9rem;">{message}</div>'
    st.markdown(html, unsafe_allow_html=True)
...
def _render_range_bar(label, low, high, current):
    ...
    html_content = f"""
    <div style="background: rgba(30, 41, 59, 0.7); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 12px; padding: 15px; box-shadow: 0 4px 6px rgba(0,0,0,0.3); margin-bottom: 10px;">
        <h4 style="color: #FFFFFF !important; margin-top:0; margin-bottom:15px; font-size:1rem;">🎯 {label} Range</h4>
        <div style="display:flex; justify-content:space-between; font-size:0.8rem; color:#94A3B8 !important; margin-bottom:8px;">
        ...
        <div style="text-align:center; font-size:0.9rem; margin-top:16px; color:#F8FAFC !important;">
            Current: <span style="color:#D4AF37 !important; font-weight:bold;">₹{current:,.2f}</span>
        </div>
    </div>
    """
...
def render_dashboard():
    # 1. HEADER
    st.markdown('<h1 style="font-size: 2.2rem; font-weight: 800; background: linear-gradient(90deg, #D4AF37, #FFF5D1, #C5A059); -webkit-background-clip: text; -webkit-text-fill-color: transparent; text-align: center; margin-bottom: 0px; padding-top: 10px;">🚩 STOCK BY DSD AI</h1>', unsafe_allow_html=True)
    st.markdown('<p style="text-align: center; color: #94A3B8 !important; font-size: 0.9rem; margin-bottom: 20px;">Professional AI Market Intelligence</p>', unsafe_allow_html=True)
    
    # 2. MAIN SEARCH BAR (Moved from Sidebar)
    symbol = st.selectbox("🔍 Search F&O Stock", options=FO_STOCKS_LIST, index=FO_STOCKS_LIST.index("RELIANCE"))
    
    # 3. LIVE GAINERS & LOSERS (Moved from Sidebar)
    with st.expander("🔥 Live F&O Action (Top Gainers & Losers)", expanded=False):
        ...
        if gainers or losers:
            ...
            with trend_tab1:
                if gainers:
                    for g in gainers:
                        st.markdown(f"""
                        <div style="background:rgba(30, 41, 59, 0.7); border: 1px solid rgba(255,255,255,0.05); border-radius: 8px; padding: 10px; margin-bottom: 8px;">
                            <div style="display:flex; justify-content:space-between; align-items:center;">
                                <span style="color:#FFFFFF !important; font-weight:bold; font-size:0.9rem;">{g['symbol']}</span>
                                <span style="color:#10B981 !important; font-weight:bold; font-size:0.85rem;">+{g['pct']}%</span>
                            </div>
                            <div style="color:#94A3B8 !important; font-size:0.8rem; margin-top:2px;">₹{g['current']:,.2f}</div>
                        </div>
                        """, unsafe_allow_html=True)
            ...
            with trend_tab2:
                if losers:
                    for ls in losers:
                        ...
                        <div style="background:rgba(30, 41, 59, 0.7); border: 1px solid rgba(255,255,255,0.05); border-radius: 8px; padding: 10px; margin-bottom: 8px;">
                            <div style="display:flex; justify-content:space-between; align-items:center;">
                                <span style="color:#FFFFFF !important; font-weight:bold; font-size:0.9rem;">{ls['symbol']}</span>
                                <span style="color:#EF4444 !important; font-weight:bold; font-size:0.85rem;">{ls['pct']}%</span>
                            </div>
                            <div style="color:#94A3B8 !important; font-size:0.8rem; margin-top:2px;">₹{ls['current']:,.2f}</div>
                        </div>
                        """, unsafe_allow_html=True)
    ...
    if indices:
        col1, col2 = st.columns(2) # 2 columns for mobile
    ...
    # 5. STOCK ANALYSIS
    if symbol:
        st.markdown(f'<h2 style="color:#FFFFFF !important; font-size:1.5rem;">💎 Analysis: <span style="color:#D4AF37 !important;">{symbol}</span></h2>', unsafe_allow_html=True)
        ...
        with tab2:
            try:
                ...
                st.markdown(f'<div style="background:rgba(30,41,59,0.5); padding:12px; border-radius:10px; border:1px solid rgba(255,255,255,0.05); margin-bottom:10px;"><div style="color:#94A3B8 !important; font-size:0.85rem;">Market Cap</div><div style="color:#FFFFFF !important; font-size:1.2rem; font-weight:bold;">₹{mcap/10000000:,.2f} Cr</div></div>', unsafe_allow_html=True)
                
                pe = info.get('trailingPE', 0)
                pe_color = "🟢" if pe and pe < 25 else "🔴" if pe and pe > 40 else "⚪"
                st.markdown(f'<div style="background:rgba(30,41,59,0.5); padding:12px; border-radius:10px; border:1px solid rgba(255,255,255,0.05); margin-bottom:10px;"><div style="color:#94A3B8 !important; font-size:0.85rem;">{pe_color} P/E Ratio</div><div style="color:#FFFFFF !important; font-size:1.2rem; font-weight:bold;">{round(pe,2) if pe else "N/A"}</div></div>', unsafe_allow_html=True)
                
                roe = info.get('returnOnEquity', 0) * 100 if info.get('returnOnEquity') else 0
                roe_color = "🟢" if roe > 15 else "🔴" if roe < 5 else "⚪"
                st.markdown(f'<div style="background:rgba(30,41,59,0.5); padding:12px; border-radius:10px; border:1px solid rgba(255,255,255,0.05); margin-bottom:10px;"><div style="color:#94A3B8 !important; font-size:0.85rem;">{roe_color} ROE</div><div style="color:#FFFFFF !important; font-size:1.2rem; font-weight:bold;">{round(roe,2)}%</div></div>', unsafe_allow_html=True)
            except: pass
                
        with tab3:
            try:
                ticker = yf.Ticker(f"{symbol}.NS")
                st.markdown('<h3 style="color:#FFFFFF !important; font-size:1.2rem;">🚨 Promoter Radar</h3>', unsafe_allow_html=True)
                try:
                    insider = ticker.insider_transactions
                    if insider is not None and not insider.empty: _render_alert("🔴 **ALERT:** प्रमोटर गतिविधि दर्ज की गई है।", "error")
                    else: _render_alert("🟢 **SAFE:** प्रमोटर द्वारा शेयर बेचने का कोई सिग्नल नहीं है।", "success")
                except: _render_alert("🟢 **SAFE:** प्रमोटर का कोई अलर्ट नहीं है।", "success")
                ...
                if major_holders is not None and not major_holders.empty:
                    ...
                    df_m["Category"] = df_m["Category"].replace({"insidersPercentHeld": "👔 Promoter Holding", "institutionsPercentHeld": "🏦 Inst. Holding", "institutionsFloatPercentHeld": "🌊 Float by Inst.", "institutionsCount": "🏢 Total Inst."})
        ...
        with st.expander("🔥 DSD 52-Week High Master Radar", expanded=True):
            st.markdown('<p style="color:#94A3B8 !important; font-size:0.85rem;">इस लिस्ट में F&O स्टॉक्स दिखेंगे जो 1 साल के उच्चतम स्तर के करीब या ऊपर हैं।</p>', unsafe_allow_html=True)
            with st.spinner("Scanning ALL F&O stocks live..."):
                breakout_df = scan_52w_high_stocks()
                if not breakout_df.empty:
                    _render_alert(f"🟢 🚨 कुल {len(breakout_df)} F&O स्टॉक्स हाई पर हैं!", "success")
                    st.dataframe(breakout_df, use_container_width=True, hide_index=True)
                else:
                    _render_alert("🔴 कोई भी स्टॉक हाई पर नहीं है
