...
Ultra Premium Dark Mode UI (Custom HTML Built)
"""
...
<style>
    ...
    .stSelectbox div[data-baseweb="select"] span {
        color: #000000 !important; /* Keep dropdown text dark so it's readable when open */
    }
</style>
...
# --- UI COMPONENTS (Custom HTML to force visibility) ---

def _render_custom_metric(label, value, delta):
    ...
    <div style="background: rgba(30, 41, 59, 0.7); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 12px; padding: 15px; box-shadow: 0 4px 6px rgba(0,0,0,0.3);">
        ...
        <div style="color: {color} !important; font-size: 0.85rem; margin-top: 5px; font-weight: bold;">{delta}</div>
    </div>
    """
...
def render_market_overview():
    try:
        ...
        data1 = indices_data.get("NIFTY50", {})
        with col1: _render_custom_metric("NIFTY 50", f"{data1.get('value', 0):,.2f}", f"↑ +{data1.get('change', 0):,.2f} ({data1.get('change_percent', 0):.2f}%)")
        
        data2 = indices_data.get("SENSEX", {})
        with col2: _render_custom_metric("SENSEX", f"{data2.get('value', 0):,.2f}", f"↑ +{data2.get('change', 0):,.2f} ({data2.get('change_percent', 0):.2f}%)")
        
        data3 = indices_data.get("BANKNIFTY", {})
        with col3: _render_custom_metric("BANK NIFTY", f"{data3.get('value', 0):,.2f}", f"↑ +{data3.get('change', 0):,.2f} ({data3.get('change_percent', 0):.2f}%)")
        
        data4 = indices_data.get("NIFTYIT", {})
        with col4: _render_custom_metric("NIFTY IT", f"{data4.get('value', 0):,.2f}", f"↑ +{data4.get('change', 0):,.2f} ({data4.get('change_percent', 0):.2f}%)")
    except Exception:
        pass
...
def render_dashboard():
    ...
    if symbol:
        ...
        with col1:
            _render_range_bar("Intraday", 2450.00, 2530.00, 2495.50) # Illustrative Live
        with col2:
        ...
        with tab2:
            try:
                ...
                f_col1.markdown(f'<div style="background:rgba(30,41,59,0.5); padding:15px; border-radius:10px; border:1px solid rgba(255,255,255,0.05);"><div style="color:#94A3B8 !important; font-size:0.9rem;">Market Cap</div><div style="color:#FFFFFF !important; font-size:1.4rem; font-weight:bold;">₹{mcap/10000000:,.2f} Cr</div></div>', unsafe_allow_html=True)
                
                pe = info.get('trailingPE', 0)
                pe_color = "🟢" if pe < 25 else "🔴" if pe > 40 else "⚪"
                f_col2.markdown(f'<div style="background:rgba(30,41,59,0.5); padding:15px; border-radius:10px; border:1px solid rgba(255,255,255,0.05);"><div style="color:#94A3B8 !important; font-size:0.9rem;">{pe_color} P/E Ratio</div><div style="color:#FFFFFF !important; font-size:1.4rem; font-weight:bold;">{round(pe,2) if pe else "N/A"}</div></div>', unsafe_allow_html=True)
                
                roe = info.get('returnOnEquity', 0) * 100 if info.get('returnOnEquity') else 0
                roe_color = "🟢" if roe > 15 else "🔴" if roe < 5 else "⚪"
                f_col3.markdown(f'<div style="background:rgba(30,41,59,0.5); padding:15px; border-radius:10px; border:1px solid rgba(255,255,255,0.05);"><div style="color:#94A3B8 !important; font-size:0.9rem;">{roe_color} ROE</div><div style="color:#FFFFFF !important; font-size:1.4rem; font-weight:bold;">{round(roe,2)}%</div></div>', unsafe_allow_html=True)
            except: pass
        ...
        with st.expander("🔥 DSD 52-Week High Master Radar", expanded=True):
            ...
            with st.spinner("Scanning ALL F&O stocks live..."):
                ...
                if not breakout_df.empty:
                    _render_alert(f"🟢 🚨 कुल {len(breakout_df)} F&O स्टॉक्स आज ज़बरदस्त तेज़ी (Breakout) में हैं!", "success")
