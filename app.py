...
# -------------------------------------------------------------------
# FUNCTON 1: FII / DII Data Fetcher (Smart Fallback Tracker)
# -------------------------------------------------------------------
@st.cache_data(ttl=3600)
def fetch_fii_dii_data():
    try:
        # Live HTML Scraping from Moneycontrol / StockEdge fallback
        url = "https://www.moneycontrol.com/stocks/marketstats/fii_dii_activity.php"
    ...
    # Fallback Data if NSE/Moneycontrol blocks connection
...
def get_recent_results():
    # Mock dynamic data of highly tracked Nifty stocks results
    results_data = [
...
# -------------------------------------------------------------------
# SIDEBAR CONTROLS (Stock Selection & Indicators)
# -------------------------------------------------------------------
st.sidebar.header("DSD STOCK TALK™ Controls")

# Stock Input
ticker = st.sidebar.text_input("Enter NSE Ticker (e.g. RELIANCE.NS, SBIN.NS)", value="RELIANCE.NS")
...
# -------------------------------------------------------------------
# DASHBOARD LAYOUT: ROW 1 (FII/DII & Corporate Results)
# -------------------------------------------------------------------
...
with col_res:
    ...
    # Custom colored styling for Good/Bad results
...
# -------------------------------------------------------------------
# DASHBOARD LAYOUT: ROW 2 (Live Interactive Candlestick Chart)
# -------------------------------------------------------------------
...
try:
    ...
    if not df.empty:
        ...
        df['BB_std'] = df['Close'].rolling(window=20). his_std() if hasattr(df['Close'].rolling(window=20), 'std') else df['Close'].rolling(window=20).mean().pct_change() # Safetynet
        ...
        # Remove empty weekend spaces from chart
        fig.update_xaxes(rangebreaks=[dict(bounds=["sat", "mon"])])
        
        st.plotly_chart(fig, use_container_width=True)
        
        # Fundamental Snapshot Component
